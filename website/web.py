...
PROFANE_WORDS = set(w.lower() for w in pd.read_csv(file_prefix + "helpers/profane_words.txt", header=None)[0].tolist() + ['condom'] + ['fleshlight'])
...
def consent_form():
    ...
    elif request.query_string.decode() == "from=reddit&sub=internet":
        return render_template('expr_done.html')
