...
bold_labels <- ifelse(levels(factor(filtered_df
$aut_item)) %in% cluster, "bold", "plain")

ggplot(data=filtered_df
, aes(y=reorder(aut_item, score), x=score)) + 
  geom_boxplot() +
  labs(x = "Score", y = "AUT Item") +
  ggtitle("Distribution of Originality Scores on AUT Items") +
  theme_few() +
  scale_y_discrete(labels = levels(factor(filtered_df
$aut_item)), breaks = levels(factor(filtered_df
$aut_item))) + # Add this line to control y-axis labels
  theme(axis.text.y = element_text(face = bold_labels)) # Add this line to apply bold labels conditionally
ggsave(file.path("../../plots/prior_work_aut_dist_highlight.png"), dpi = 300, bg = 'transparent')
